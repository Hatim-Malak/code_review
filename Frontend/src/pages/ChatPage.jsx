...
import { User, Bot, Send, Code, Sparkles, Zap, ShieldAlert } from "lucide-react";
...
const ChatPage = () => {
  ...
  const layout = useMemo(() => {
    return <CustomNavbar logo="./ai.png" items={navItems} />;
  }, [navItems]);
  ...
  return (
    <div className="bg-cream flex flex-col h-screen w-full relative">
      ...
      <div className="flex-1 overflow-auto scrollable pt-28 pb-32 px-4 flex justify-center" ref={chatContainerRef}>
                    ...
                    {chat.AI_message ? (
                      parseAIMessage(chat.AI_message).map((part, idx) =>
                        part.type === "text" ? (
                          <p
                            key={idx}
                            className="text-[15px] text-greenDark font-medium leading-relaxed whitespace-pre-wrap"
                          >
                            {part.content}
                          </p>
                        ) : (
      ...
      {/* Premium Prompt Bar */}
      <div className="w-full fixed bottom-0 z-20 bg-gradient-to-t from-cream via-cream to-transparent pb-6 pt-10 px-4">
        <form
          onSubmit={handleSend}
